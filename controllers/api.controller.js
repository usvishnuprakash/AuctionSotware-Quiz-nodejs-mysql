...
module.exports = {
  ...
  async manageSortTypes(req, res, next) {
    ...
    if (req.params.sortType === "recentProjects") {
      query = `
      SELECT projects.project_id, projects.project_title,users.username,projects.date_added AS date 
      ...
      OFFSET ${(req.body.page || 1) * req.body.limit - req.body.limit}
      `;
    }
    if (req.params.sortType === "orderByCategoryNameAsc") {
      ...
      query = `
      SELECT projects.project_id, projects.project_title,users.username,projects.date_added AS date , categories.category_name 
      FROM ilance_projects AS projects 
      LEFT JOIN ilance_users AS users ON users.user_id=projects.user_id 
    ...
    if (req.params.sortType === "orderByUsernameAsc") {
      query = `
      SELECT projects.project_id, projects.project_title , users.username , users.user_id FROM ilance_projects AS projects
      LEFT JOIN ilance_users as users on users.user_id=projects.user_id
